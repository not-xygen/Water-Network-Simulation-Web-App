  ...
  simulationInterval = setInterval(() => {
    try {
      ...
      const newEdges = edges.map((edge) => {
        ...
        else sourcePressure = sourceNode.outletPressure ?? 0;

        if (targetNode.type === "tank")
          targetPressure = targetNode.currentVolumeHeight ?? 0;
        else targetPressure = targetNode.outletPressure ?? 0;
        ...
        if (targetNode.type === "pump") {
          ...
          const safeSourcePressure =
            sourceNode.type === "fitting"
              ? sourceNode.outletPressure ?? sourceNode.outletPressure ?? 0
      ...
      const nextNodes = initialNodes.map((node) => {
        ...
        if (node.type === "fitting") {
          ...
          if (inletPressure === 0) {
            inletPressure = node.outletPressure || 0;
          }
          ...
          const adjustedFlow = Math.max(0, totalFlow);
        ...
        if (node.type === "valve") {
          ...
          const loss = node.minorLossCoefficient ?? 0.05;
          const pressure = Math.max(0, node.outletPressure - loss);
